...
def profile_fit_x1(
    ...
    profiles: dict,
    same_artist: np.ndarray,
    highly_niche: np.ndarray | None = None,
    discovery: np.ndarray | None = None,
) -> np.ndarray:
    """X1: 인기곡 항을 유형에 따라 조절한다. discovery_score는 곱하지 않는다.

    중간 플리는 spread로 부드럽게 줄이고, Highly niche는 인기곡 항을 유지,
    Discovery-oriented는 일괄 하향을 끈다. (gpt.md 3.4 if-then)
    """
    niche = zscore_1d(-profiles["avg_logpop"])
    centric = zscore_1d(profiles["top_artist_share"])
    spread_z = zscore_1d(profiles["popularity_spread"])
    gate = 1.0 - 1.0 / (1.0 + np.exp(-np.clip(spread_z, -20.0, 20.0)))
    if highly_niche is not None:
        gate = np.where(highly_niche, 1.0, gate)
    if discovery is not None:
        gate = np.where(discovery, 0.0, gate)
    cand_logpop = profiles["logpop"][candidates]
...
def main():
    ...
    if purpose_map:
        ...
        n_matched = sum(
            1 for pid in playlist_ids if name_key(names.get(pid, "")) in purpose_map
        )
        print(
            f"purpose name_key matched {n_matched:,}/{len(playlist_ids):,}  "
            f"axis on: {sum(c is not None for c in purpose_cats):,}  "
    ...
    print(
        ...
        f"(type override: niche=1, discovery=0)"
    )
    ...
    if n_purpose_axis:
        ...
        print(
            "P1 skipped: no mapped title purpose categories. "
            "Drive에 purpose_titles/outputs/all_unique_purpose_labels.csv 가 "
            "PROJECT_DIR 아래에 있는지, 노트북이 --purpose-labels 를 넘기는지 확인하세요."
        )
    ...
    x1_selected, _, _, x1_fallback = select_lambda(
        x1_rows, ["highly_niche", "discovery"], k, b0_overall
    )
    # C1과 같은 λ에서만 공식을 바꾼다. pair-guard / λ=0 선택이면 X1이 B0이 된다.
    x1_ctx = c1_ctx
    x1_frozen = True
    ...
    if val_pur is not None:
        ...
        p1_selected, _, _, _ = select_lambda(p1_rows, p1_target, k, b0_overall)
        p1_on = [(lam, art, res) for lam, art, res in p1_rows if lam > 0]
        p1_pur, _, _, p1_fallback = select_lambda(p1_on, p1_target, k, b0_overall)
    ...
    print(
        ...
        f"X1 used=({x1_ctx}) selected=({x1_selected}) frozen_to_c1={x1_frozen}  "
        f"P1=({x1_ctx}, {p1_pur}) selected_pur=({p1_selected}) fallback={p1_fallback}"
    )
    ...
    output = {
        ...
        "lambda": {
            ...
            "X1": {
                "ctx": x1_ctx,
                "selected": x1_selected,
                "frozen_to_c1": x1_frozen,
    ...
    output_path = args.output or args.checkpoint.with_name(
        f"{args.checkpoint.stem}_context_rerank_n{args.candidate_n}_x1p1_on.json"
    )
